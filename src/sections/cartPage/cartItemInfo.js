...
import BackdropLoading from "@/components/loading-screen/backdropLoading";
...
const CartItemInfo = ({ data = {}, formik, checkoutFormik }) => {
  ...
  const { cartState, getMyCartData, setCartRemarkVal } = useCartContext();
  const { isCartLoading, cartRemarkVal } = cartState;
  const { modificationUser } = cookies || {};
  let head_sys_id = modificationUser?.head_sys_id
    ? modificationUser?.head_sys_id
    : 0;
  const { cartCityList } = useSelector(
    (state) => state.cartPage
  );
  const { deliveryPolicyFun } = cartState;
  ...
  const handleDialogueClose = useCallback((key) => {
    setDialogueState((prevState) => ({
      ...prevState,
      [key]: { open: false, data: prevState[key]?.data },
    }));
  }, []);
  ...
  const handleCartRemarkVal = (newValue) => {
    setCartRemarkVal(newValue);
  };
  return (
    <Box
      component="div"
    ...
    >
                    ...
                    <React.Fragment>
                      ...
                      <Box mt={2}>
                        <TextBox
                          ...
                          multiline={true}
                          value={cartRemarkVal}
                          onChange={(e) => handleCartRemarkVal(e.target.value)}
                        />
                      </Box>
                      <SectionThree
      ...
      {deliveryOpen?.data?.policyDataList && (
        <CartDilogue
          title={deliveryOpen.data.policyDataList?.title}
        ...
        >
          <Typography
            ...
            dangerouslySetInnerHTML={{
              __html: deliveryOpen.data.policyDataList?.description,
            }}
          />
        </CartDilogue>
      )}
