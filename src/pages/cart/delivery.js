import DeliverySkeleton from "@/components/skeleton/checkout/delivery";
...
const DeliverySection = dynamic(() => import("@/sections/delivery"), {
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) => async (context) => {
    ...
    const { user, USER_ID } = GET_ALL_COOKIES || {};

    await setReduxCookies(context, store);

    const { JWTAuthToken, modificationUser } = GET_ALL_COOKIES;
    ...
    if (user) {
      ...
      await dispatch(getDeliveryPageData());
    }
